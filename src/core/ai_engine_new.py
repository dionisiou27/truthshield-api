...
class TruthShieldAI:
    ...
    async def _search_sources(self, query: str) -> List[Source]:
        ...
        try:
            ...
            tasks = [
                ...
                self._search_gdelt(enhanced_query)
            ]

            wikipedia_results, wikidata_results, dbpedia_results, google_results, snopes_results, factcheck_results, politifact_results, mimikama_results, correctiv_results, euvsdisinfo_results, pubmed_results, core_results, news_results, reuters_results, dw_results, rss_results, gdelt_results = await asyncio.gather(
            ...
            for i, result in enumerate([
                wikipedia_results, wikidata_results, dbpedia_results, google_results, snopes_results, factcheck_results, politifact_results, mimikama_results, correctiv_results, euvsdisinfo_results, pubmed_results, core_results, news_results, reuters_results, dw_results, rss_results, gdelt_results
            ]):
                name = method_names[i]
            ...
            all_sources.extend(extend_safe(gdelt_results))
    ...
    async def _search_gdelt(self, query: str) -> List[Source]:
        """Search GDELT for global events and news"""
        try:
            # GDELT 2.0 API for recent events
            ...
            async with httpx.AsyncClient(timeout=15.0) as client:
                response = await client.get(url, params=params, headers={"User-Agent": "TruthShield/1.0"})
                if response.status_code == 200:
                    data = response.json()
                    sources = []
                    
                    # GDELT returns articles with metadata
                    for article in data.get("articles", [])[:10]:
                        title = article.get("title", "")
                        url_article = article.get("url", "")
                        snippet = article.get("snippet", "")
                        
                        # Extract source and tone information
                        source_name = article.get("seendomain", "Unknown source")
                        tone = article.get("tone", {})
                        tone_score = tone.get("tone", 0) if tone else 0
                        
                        # Calculate credibility based on tone and source
                        credibility_score = 0.7  # Base score for news
                        if tone_score > 0:
                            credibility_score += 0.1  # Positive tone
                        elif tone_score < -5:
                            credibility_score -= 0.1  # Very negative tone
                        
                        # Boost credibility for known reliable sources
                        reliable_domains = ["reuters.com", "ap.org", "bbc.com", "nytimes.com", 
                                          "washingtonpost.com", "theguardian.com", "dw.com"]
                        if any(domain in source_name.lower() for domain in reliable_domains):
                            credibility_score += 0.1
                        
                        credibility_score = min(credibility_score, 0.95)
                        
                        if url_article and title:
                            sources.append(Source(
                                url=url_article,
                                title=title[:180],
                                snippet=f"{source_name}: {snippet[:200]}" if snippet else f"GDELT event from {source_name}",
                                credibility_score=credibility_score,
                                date_published=article.get("datetime")
                            ))
